...
const Word = () => {
  ...
  const fetchWordDetails = async (text) => {
    ...
    try {
      ...
      const partsOfSpeech = [
        ...new Set(data[0]?.meanings?.map((m) => m.partOfSpeech) || []),
      ];
      const shortForms = partsOfSpeech
        .map((pos) => getShortForm(pos))
        .join(", ");
      ...
      const synonyms = [
        ...new Set(
          data[0]?.meanings?.flatMap(
            (m) => m.definitions?.flatMap((d) => d.synonyms || []) || []
          ) || []
        ),
      ];
  ...
  const fetchFriendWords = async (friendUserId) => {
    ...
    try {
      ...
      setWordList((prev) => [
        ...prev,
        ...uniqueWords.map((word) => ({ ...word, key: word._id })),
      ]);
  ...
  const handleWordEdit = async (values) => {
    try {
      ...
      setWordList((prev) =>
        prev.map((item) =>
          item._id === editingWord._id
            ? { ...updatedWord, key: updatedWord._id }
            : item
        )
      );
